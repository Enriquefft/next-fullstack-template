...
- `src/lib/polar.ts` – Polar SDK client
- Configuration via `POLAR_ACCESS_TOKEN` and `POLAR_MODE` (sandbox/production)
- Integrated with Better Auth for user management
- Product catalog, checkout, and customer portal functionality
...
- Configure with `NEXT_PUBLIC_POSTHOG_KEY` and `POSTHOG_PROJECT_ID`
...
- Configure with `UPLOADTHING_TOKEN`
...
**Google Maps** integration for address autocomplete and location features.

- Uses `@react-google-maps/api` package
- `src/components/AddressAutocomplete.tsx` – Example address autocomplete component
- `src/hooks/use-google-places.tsx` – Custom hook for Places API